 ...
 * set up a shared MyBatis SqlSessionFactory in a Spring application context;
 ...
 * for transaction demarcation in combination with a SqlSessionFactory. JTA
 * should be used for transactions which span multiple databases or when
 * container managed transactions (CMT) are being used.
 *
 ...
 * @see #setDataSource
 * @version $Id: SqlSessionFactoryBean.java 3266 2010-11-22 06:56:51Z
 *          simone.tripodi $
 */
public class SqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>,
	...
	private String mapperPackageNames[];
	...
	protected SqlSessionFactory buildSqlSessionFactory() throws IOException,
			...
			for (Resource mapperLocation : this.mapperLocations) {
				...
				try {
					...
					XMLMapperBuilder xmlMapperBuilder = new XMLMapperBuilder(
							reader, configuration, path, sqlFragments);
		...
		if (!mapper) {
			if (this.logger.isDebugEnabled()) {
				this.logger
						.debug("Property 'mapperLocations' or 'mapperPackageName' was not specified, only MyBatis mapper files specified in the config xml were loaded");
			}
		}
	...
	public void setMapperPackageNames(String[] mapperPackageNames) {
		this.mapperPackageNames = mapperPackageNames;
	}
