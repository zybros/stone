...
public class ProductServiceImpl extends MybatisBaseServiceImpl implements IProductService {
	...
	public void addProductSku(TProductSku sku) throws Exception {
		...
		List<TSkuValue> valList = sku.getValList();
		for (TSkuValue val : valList) {
			val.setProductId(sku.getProductId());
			val.setSkuId(sku.getId());
		}
		// 插入商品sku的规格属性值
		TSkuValueMapper svm = this.getMapper(TSkuValueMapper.class);
